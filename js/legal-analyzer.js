class LegalAnalyzer {
  ...
  loadLegalRules() {
    return {
      ...
      formalErrors: [
        ...
        {
          id: 'falta_identificacion',
          description: 'Identificación insuficiente del contribuyente',
          pattern: text => !text.match(/\b[0-9A-Z][0-9]{7}[A-Z]\b|\b[ABCDEFGHJKLMNPQRSUVW][0-9]{7}[0-9A-J]\b/g),
          severity: 'high',
          legalBasis: 'Art. 9 Ley 58/2003 General Tributaria'
        }
      ],
      ...
      proceduralErrors: [
        ...
        {
          id: 'notificacion_defectuosa',
          description: 'Notificación defectuosa',
          pattern: text => !text.toLowerCase().includes('notificado') && !text.toLowerCase().includes('recibido'),
          severity: 'high',
          legalBasis: 'Art. 40-46 Ley 39/2015 PACAP'
        },
        {
          id: 'falta_motivacion',
          description: 'Falta de motivación suficiente',
          pattern: text => text.length < 500 && text.includes('sanción'),
          severity: 'medium',
          legalBasis: 'Art. 35 Ley 39/2015 PACAP'
        }
      ],
      ...
      calculationErrors: [
        {
          id: 'error_calculo_intereses',
          description: 'Posible error en el cálculo de intereses de demora',
          pattern: text => text.includes('interés') && text.includes('demora'),
          severity: 'medium',
          legalBasis: 'Art. 26 Ley 58/2003 General Tributaria'
        },
        {
          id: 'error_calculo_sancion',
          description: 'Posible error en el cálculo de la sanción',
          pattern: text => text.includes('sanción') && text.includes('%'),
          severity: 'medium',
          legalBasis: 'Art. 191-206 Ley 58/2003 General Tributaria'
        }
      ]
    };
  }
  ...
  identifyProcedureType(documentData) {
    ...
    } else if (documentType === 'liquidacion') {
      if (text.toLowerCase().includes('iva')) {
        return 'liquidacion_iva';
      } else if (text.toLowerCase().includes('irpf')) {
        return 'liquidacion_irpf';
      } else {
        return 'liquidacion_generica';
      }
    } else if (documentType === 'requerimiento') {
      if (text.toLowerCase().includes('información')) {
        return 'requerimiento_informacion';
      } else {
  ...
  async detectErrors(documentData, procedureType) {
    ...
    this.legalRules.calculationErrors.forEach(rule => {
      if (rule.pattern(text)) {
        errors.push({
          type: 'calculation',
          id: rule.id,
